...
export function PromocionesView() {
  ...
  const handleSave = () => {
    ...
    const promo: ProductPromotion = {
      ...
      valor_secundario: form.tipo === "nxm" ? parseInt(form.valor_secundario) : undefined,
      activa: form.activa,
    }
    setProducts((prev) =>
      prev.map((p) => (p.id === selected.id ? { ...p, promotion: promo } : p)),
    )
    setModalOpen(false)
    setSelected(null)
  ...
  const handleDelete = () => {
    if (!selected) return
    setProducts((prev) =>
      prev.map((p) => (p.id === selected.id ? { ...p, promotion: undefined } : p)),
    )
    setDeleteOpen(false)
    setSelected(null)
